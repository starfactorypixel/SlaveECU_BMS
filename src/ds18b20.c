...
uint8_t ds_reset_pulse(uint16_t PinMask)
{
   ...
   if((DS_PORT->IDR & PinMask)==0)  return 2;         //проверить линию на отсутствие замыкани€
   DS_PORT->ODR &= ~PinMask;                          //пот€нуть шину к земле
...
uint8_t ds18b20_Reset(void)
{
  ...
  status = GPIOB->IDR & GPIO_DS_PIN_READ;//провер€ем уровень
  DelayMicro(500);//задержка как минимум на 480 микросекунд
...
uint8_t ds18b20_ReadBit(void)
{
  ...
  DelayMicro(2);
	GPIOB->ODR |= GPIO_DS_PIN_OUT;//высокий уровень
	DelayMicro(13);
	bit = (GPIOB->IDR & GPIO_DS_PIN_READ ? 1 : 0);//провер€ем уровень	
	DelayMicro(45);
  return bit;
}
//-----------------------------------------------
